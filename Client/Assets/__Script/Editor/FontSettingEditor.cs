...
{
    [MenuItem("Window/Font Setting")]
    static void Open() => GetWindow<FontSettingEditor>("Font Setting").minSize = new Vector2(180, 30);
    ...
    private void OnGUI()
    {
        ...
        if (globalFontSize < 0)
            globalFontSize = EditorStyles.miniLabel.fontSize;
        ...
        if (delta != 0 && EditorStyles.miniLabel.fontSize + delta > 0)
        {
            ...
            {
                if (!ValidFontStyle(x) || styles.Contains(x))
                    continue;
