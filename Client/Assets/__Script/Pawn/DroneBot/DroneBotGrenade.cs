...
namespace Game
{
    public class DroneBotGrenade : ProjectileMovement, IObjectPoolable
    {
        ...
        protected override void AwakeInternal()
        {
            ...
            onHitSomething += (c) =>
            {
                if (c.gameObject.layer == LayerMask.NameToLayer("Obstacle"))
                    Explode();
                else if (c.gameObject.TryGetComponent<PawnColliderHelper>(out var helper) && helper.pawnBrain != emitterBrain.Value && helper.pawnBrain != GameContext.Instance.playerCtrler.possessedBrain)
                    Explode();
            };
