...
namespace FIMSpace.Generating.Planning.PlannerNodes.Generating
{
    public class PR_FindPathTowards : PlannerRuleBase
    {
        ...
        //[Tooltip("Moving 'Towards End Cell' to 'PATH Start Cell' port in case when path length is zero (can happen when checkers are aligning)")]
        //[HideInInspector, SerializeField] protected bool PathLengthZeroFix = false;
        [Tooltip("When rooms are aligning and path length is zero, you can mark such result as fail and then call commands generating between 'Start Cell' and 'Towards Cell'")]
        [HideInInspector, SerializeField] protected bool PathLengthZeroIsFail = false;
        ...
        public override void Execute(PlanGenerationPrint print, PlannerResult newResult)
        {
            ...
            if (path != null)
            {
                ...
                {
                    //From_StartCell.Checker.DebugLogDrawCellInWorldSpace(From_StartCell.Cell, Color.magenta);
                    //UnityEngine.Debug.DrawLine(Towards_EndCell.Checker.GetWorldPos(Towards_EndCell.Cell), From_StartCell.Checker.GetWorldPos(From_StartCell.Cell), Color.magenta, 1.01f);
                    //UnityEngine.Debug.Log("PathStart == Start ? " + (From_StartCell.Cell == Path_StartCell.Cell) + " PathEnd == End ? " + (Path_EndCell.Cell == Towards_EndCell.Cell) + " PathStart null: " + (Path_StartCell.Cell == null) + " PathEnd null: " + (Path_EndCell.Cell == null) + " from null: " + (From_StartCell.Cell == null) + " to null: " + (Towards_EndCell.Cell == null));
                    ...
                    {
                        //UnityEngine.Debug.Log(From_StartCell.Checker.ParentPlanner.ArrayNameString + " Cell 1 WPosition = " + From_StartCell.Checker.LocalToWorld(From_StartCell.Cell.Pos) + "  " + Towards_EndCell.Checker.ParentPlanner.ArrayNameString +  " Cell 2 WPosition = " + Towards_EndCell.Checker.LocalToWorld(Towards_EndCell.Cell.Pos));
                    }
                    else
                    {
                        if (cell != null) Path_StartCell.ProvideFullCellData(cell, b.LatestChecker, b.LatestResult);
